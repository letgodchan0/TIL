...
- 로컬 주소 `http://127.0.0.1:8000/` 뒤에 `admin`이나 `index`를 입력했을 때 그에 해당하는 화면이나 활동이 이루어지는 것을  `view` 함수 내부의 함수를 통해 이루어 지는데, `index`와 그에 해당하는 `views.???` 를 매핑해주는 것을 <u>**urls.py**</u> 에 제일 먼저 작성을 한다. 왜냐면 클라이언트의 요청을 가장 먼저 받기 때문에!
- 즉, 내가 `index` 주소를 요청하면, `urls`한테 야! `view` 함수 메인페이지를 리턴하는 애 실행시켜, 실행시키면 `index.html`을 준비시키고 사용자에게 응답해주는 느낌
